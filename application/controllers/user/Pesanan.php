...
class Pesanan extends MY_controller
{
    ...
    {
        $data = [
            'index' => 'user/pesanan/index',
            'index_js' => 'user/pesanan/index_js',
            'title' => 'Pesanan Saya',
        ];
    ...
    {
        $data = [];
        $html = $this->load->view('user/pesanan/form', $data, true);
    ...
    public function load()
    {
        ...
        if ($id == 'batal') {
            $all = $this->db->query("SELECT * from pesanan where status in ('4','5') and id_user='$this->id_akun' and deleted is null ")->result();
        } else {
            $all = $this->db->query("SELECT * from pesanan where status='$id' and id_user='$this->id_akun' and deleted is null ")->result();
        }

        $data['all'] = $all;
        $data['status'] = $id;
        $html = $this->load->view('user/pesanan/list', $data, true);
